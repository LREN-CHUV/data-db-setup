...
```console
$ docker run -i -t --rm -e FLYWAY_HOST=`hostname` hbpmip/data-db-setup:2.1.7 migrate
```
...
```
  ...
  FROM hbpmip/data-db-setup:2.1.7 as parent-image
  ...
  # Final image
  FROM hbpmip/data-db-setup:2.1.7
